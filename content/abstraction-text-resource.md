...
**Update:** As [correctly pointed out](https://www.reddit.com/r/android_devs/comments/l3uc9r/finding_the_right_abstraction_when_working_with/) TextResource is not following the Open-Closed principle. Thanks for your feedback, I really appreciate it! We could make `TextResouce` follow the Open-Closed principle if `sealed class TextResouce` has a `abstract fun asString(r : Resources)` that all sub class implement. I personally think it is fine to sacrifice the Open-Closed principle in favor keeping the data structs lean and work with a pure (extension) function `asString(r : Resources)` that lives outside of the inheritance hierarchy. Why? Well, I see it as problematic to add a function with a parameter Resources that is only used for a subset of all subclasses (i.e. `SimpleTextResource` doesnt need this parameter add all). It just adds maintainance overhead and complexity (especially for testing) once it is part of your public API. Thus, I would not add such a method to the public API of `TextResource` but rather use a simple "translation" function as shown in this blog post.